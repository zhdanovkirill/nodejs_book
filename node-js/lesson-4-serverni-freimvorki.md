...
### Рендеринг даних (views)

Механізми шаблонів (в Express їх називають "механізмами перегляду") дозволяють вказувати структуру вихідного документа в шаблоні, використовуючи заповнювачі для даних, які будуть заповнюватися при створенні сторінки. Шаблони часто використовуються для створення HTML, але також можуть створювати інші типи документів.

У коді налаштувань програми ви задаєте механізм шаблонів для використання і місце, де Express повинен шукати шаблони, використовуючи налаштування «views» і «engine», як показано нижче:
...
Зовнішній вигляд шаблону залежатиме від того, який двигун ви використовуєте. Припускаючи, що ви маєте файл шаблону з ім'ям «index. \<Template\_extension>», який містить заповнювачі для змінних даних з іменами «title» та «message», ви повинні викликати `Response.render ()` у функції обробника маршруту для створення та надсилання відповіді HTML:
...
Node.js Express має готові функції обробки HTTP запитів, причому для кожного методу HTTP є своя функція, що особливо зручно при створенні REST API. І це далеко не єдина причина використання Express. Метод use() використовується для створення проміжних обробників – Middleware.

Звичайно, Express - це дуже легка платформа для веб-додатків, тому більшість її переваг і можливостей забезпечується сторонніми бібліотеками та функціями.
